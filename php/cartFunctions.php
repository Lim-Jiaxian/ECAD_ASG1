...
function removeItem()
{
	...
	// and also the session variable for counting number of items in shopping cart.
	// Get the cart ID and product ID
	$cartid = $_SESSION["Cart"];
	$pid = $_POST["product_id"]; // Product ID sent from the form

	include_once("mysql_conn.php"); // Establish database connection handle: $conn

	// Step 1: Delete the item from ShopCartItem table
	...
	// Step 2: Reduce the session variable NumCartItem by 1
	if (isset($_SESSION["NumCartItem"])) {
		$_SESSION["NumCartItem"] = max($_SESSION["NumCartItem"] - 1, 0); // Ensure it doesn't go below 0
	}
